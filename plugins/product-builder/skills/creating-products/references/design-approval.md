...
After the foundation skills finish, propose the domain implementation before
editing product-specific code.
...
- Page and route map, including public pages, protected pages, dashboard views,
  resource detail pages, forms, and API/resource routes.
- Authentication and authorization behavior when auth is enabled.
- File storage behavior when R2 is enabled, including metadata fields, allowed
  file types when known, upload/delete lifecycle, and rollback expectations.
...
If the user changes the scope, update the affected sections and ask for approval
again. Do not implement a partially approved design unless the user explicitly
limits approval to a named subset.
...
Commit the implementation after verification succeeds, or explain any command

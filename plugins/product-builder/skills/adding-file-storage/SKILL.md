...
Use `APP_FILES` as the default R2 binding and `APP_DB` as the default D1 binding
unless the existing project already uses different binding names.
...
- Preserve existing React Router request handling and inject file storage
  services through router context.
...
1. Verify the target project is a Product Builder-style Cloudflare Workers,
   Vite, React Router, TypeScript, pnpm, D1, and Drizzle project.
2. Create or register the Cloudflare R2 bucket using

...
1. Delete storage migration files in `db/migrations/` that were generated by this skill.
2. Remove the `files` table from `app/db/schema.ts`.
3. Remove `app/services/file.service.ts`.
4. Revert `app/context.ts` to remove the `files` field.
5. Revert `workers/app.ts` to remove `FilesService` wiring.
6. Remove AWS SDK packages: `pnpm remove @aws-sdk/client-s3 @aws-sdk/s3-request-presigner`.
7. Remove the RustFS service from `docker-compose.yml` and S3 variables from `.env` and `.env.example`.
8. Reset `docs/context.json` — remove `project.storage_bucket_name` and set `capabilities.file_storage` back to `"planned"`.
...
8. Run `pnpm format`, `pnpm typecheck`, `pnpm lint`, `pnpm build`, and migration commands. Fix any failures before proceeding.
9. Write `project.storage_bucket_name` and `capabilities.file_storage = "ready"` to `docs/context.json`.

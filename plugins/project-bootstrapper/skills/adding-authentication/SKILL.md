...
1. Delete auth migration files in `db/migrations/` that were generated by this skill.
2. Remove auth table additions from `app/db/schema.ts` (`users`, `sessions`, `accounts`, `verifications`).
3. Remove `app/routes/auth.tsx`, `app/routes/login.tsx`, `app/routes/logout.tsx`, `app/routes/signup.tsx`, and the corresponding unit test files.
4. Revert `app/routes.ts` to remove the four auth route entries.
5. Revert `app/context.ts` to remove the `auth` field.
6. Revert `workers/app.ts` to remove Better Auth wiring.
7. Remove Better Auth and Conform packages: `pnpm remove better-auth @conform-to/react @conform-to/zod`.
8. Remove `AUTH_SECRET` from `.env` and the placeholder from `.env.example`.
9. Reset `docs/context.json` — set `capabilities.authentication` back to `"planned"`.

Re-running this skill after a partial apply may cause duplicate route entries in `app/routes.ts` — check and deduplicate before retrying.
...
6. Run React Router type generation so `+types/*.ts` files are available before typecheck:

```sh
pnpm exec react-router typegen
```

7. Write unit tests for the login and signup route components using [references/05-tests.md](references/05-tests.md).
8. Update project documentation using [documentation-updates.md](../../shared/references/documentation-updates.md).
9. Run `pnpm format`, `pnpm typecheck`, `pnpm lint`, `pnpm build`, and `pnpm test:unit`. Fix any failures before proceeding.
10. Write `capabilities.authentication = "ready"` to `docs/context.json`.
...
- [ ] `pnpm exec react-router typegen` run after adding routes so `+types/*.ts` files exist before typecheck.
